...
uint256 hashGenesisBlock("0x000007140b7a6ca0b64965824f5731f6e86daadf19eb299033530b1e61236e43");
...
const string strMessageMagic = "ONIX Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (fLimitFree && nFees < txMinFee)
            return error("CTxMemPool::accept() : not enough fees %s, %s" PRI64d " < %" PRI64d,
                         hash.ToString().c_str(),
                         nFees, txMinFee);
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 1)
		return 100000000 * COIN; // pre-mine
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    if (vtx[0].GetValueOut() > GetBlockValue(pindex->nHeight, nFees))
        return state.DoS(100, error("ConnectBlock() : coinbase pays too much (actual=%s" PRI64d " vs limit=%s" PRI64d ")", vtx[0].GetValueOut(), GetBlockValue(pindex->nHeight, nFees)));
...
{
    // ONIX: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
        ...
        block.nVersion = 1;
        block.nTime    = 1491940886;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1033603;
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    if (nLoaded > 0)
        printf("Loaded %i blocks from external file in %s" PRI64d "ms\n", nLoaded, GetTimeMillis() - nStart);
    return nLoaded > 0;
}
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc3, 0xb9, 0xde }; // ONIX: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ONIXMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ONIXMiner : ProcessBlock, block not accepted");
    }
