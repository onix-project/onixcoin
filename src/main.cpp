...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    int halvings = nHeight / 345600;

    // Force block reward to zero when right shift is undefined.
    if (halvings >= 33)
        return nFees;

    // Subsidy is cut in half every 345600 blocks which will occur approximately every 2 years.
    nSubsidy >>= halvings;
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
  ...
  int64 LatestBlockTime = BlockLastSolved->GetBlockTime();
    ...
    if (LatestBlockTime < BlockReading->GetBlockTime()) {
        if (BlockReading->nHeight > 330000) LatestBlockTime = BlockReading->GetBlockTime();
    }
    ...
    if (BlockReading->nHeight > 330000) {
